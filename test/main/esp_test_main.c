...
static config_wifi_t        s_config_wifi = { WIFI_TYPE_STA, "Tenda_7EC1C0", "z25251325" };
 ...
 * \return     0-成功，其它失败
 */
int process_client_request(int client_sock)
{
    bool restart = false;
    char buff[1024];
 ...
 * \return     0-成功，其它失败
 */
int process_client_connect(int listen_sock)
{
#ifdef CONFIG_EXAMPLE_IPV4
    ...
    while (process_client_request(client_sock) >= 0);
...
static void tcp_server_task(void *pvParameters)
{
    while (1)
    {
        int listen_sock = create_listen_socket(80);

        while (listen_sock > 0)
        {
            process_client_connect(listen_sock);
        }
...
void app_main()
{
    ...
    s_config_wifi.type = WIFI_TYPE_STA;
    strcpy(s_config_wifi.ssid, "Tenda_7EC1C0");
    strcpy(s_config_wifi.password, "z25251325");

    s_config_light[0].on = 1;
    s_config_light[0].color = 0;
    strcpy(s_config_light[0].id, "s0");
    strcpy(s_config_light[0].name, "灯0");

    s_config_light[1].on = 1;
    s_config_light[1].color = 1;
    strcpy(s_config_light[1].id, "s1");
    strcpy(s_config_light[1].name, "灯1");

    s_config_light[2].on = 1;
    s_config_light[2].color = 2;
    strcpy(s_config_light[2].id, "s2");
    strcpy(s_config_light[2].name, "灯2");

    s_config_light[3].on = 1;
    s_config_light[3].color = 3;
    strcpy(s_config_light[3].id, "s3");
    strcpy(s_config_light[3].name, "灯3");

    s_config_light[4].on = 0;
    s_config_light[4].color = 0;
    strcpy(s_config_light[4].id, "s4");
    strcpy(s_config_light[4].name, "灯4");

    s_config_light_count = 5;
